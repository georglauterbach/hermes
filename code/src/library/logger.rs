...
impl Logger {
    ...
    fn set_log_level<'a>(log_level: Option<log::Level>) -> &'a str {
        let level_filter = log_level.map_or(log::LevelFilter::Info, |log_level| {
    ...
    /// Set the log level. The default log level chosen
    /// if [`None`] is provided is "Info".
    ///
    /// #### Errors
    ///
    /// If the logger could not be set because it already had been set,
    /// this function returns an [`Err`].
    pub fn initialize(log_level: Option<log::Level>) -> anyhow::Result<()> {
        if let Err(error) = ::log::set_logger(&LOGGER) {
            anyhow::bail!("Could not set logger to initialize the log: {error}");
        }
        let level = Self::set_log_level(log_level);
    ...
    fn log(&self, record: &::log::Record) {
        ...
        macro_rules! log_with_color {
            ($r:expr, $g:expr, $b:expr) => {{
                ...
                println!(
                    "{} {:<5} {}",
                    chrono::offset::Local::now().format("%Y-%m-%dT%H:%M:%S%:z"), //.format("%+"),
